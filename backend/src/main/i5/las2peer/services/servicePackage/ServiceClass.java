 ...
 * 
 * Note: If you plan on using Swagger you should adapt the information below in the ApiInfo
 * annotation to suit your project. If you do not intend to provide a Swagger documentation of your
 * service API, the entire ApiInfo annotation should be removed.
 * 
 */
@Path("/example")
...
public class TemplateService extends Service {
  ...
  /**
   * Simple function to validate a user login. Basically it only serves as a "calling point" and
   * does not really validate a user (since this is done previously by LAS2peer itself, the user
   * does not reach this method if he or she is not authenticated).
   * 
   */
  @GET
  @Path("/validation")
  ...
  @ApiResponses(
      value = {@ApiResponse(code = HttpURLConnection.HTTP_OK, message = "Validation Confirmation"),
          @ApiResponse(code = HttpURLConnection.HTTP_UNAUTHORIZED, message = "Unauthorized")})
  public HttpResponse validateLogin() {
    String returnString = "";
    returnString +=
        "You are " + ((UserAgent) getActiveAgent()).getLoginName() + " and your login is valid!";
  ...
  /**
   * Example method that shows how to retrieve a user email address from a database and return an
   * HTTP response including a JSON object.
   * 
   * WARNING: THIS METHOD IS ONLY FOR DEMONSTRATIONAL PURPOSES!!! IT WILL REQUIRE RESPECTIVE
   * DATABASE TABLES IN THE BACKEND, WHICH DON'T EXIST IN THE TEMPLATE.
   * 
   */
  @GET
  ...
  @Produces(MediaType.TEXT_PLAIN)
  @ApiResponses(
      value = {@ApiResponse(code = HttpURLConnection.HTTP_OK, message = "Update Confirmation"),
          @ApiResponse(code = HttpURLConnection.HTTP_UNAUTHORIZED, message = "Unauthorized"),
          @ApiResponse(code = HttpURLConnection.HTTP_INTERNAL_ERROR,
              message = "Internal Server Error")})
  @ApiOperation(value = "setUserEmail",
      notes = "Example method that changes a user email address in a database."
          + " WARNING: THIS METHOD IS ONLY FOR DEMONSTRATIONAL PURPOSES!!! "
          + "IT WILL REQUIRE RESPECTIVE DATABASE TABLES IN THE BACKEND, WHICH DON'T EXIST IN THE TEMPLATE.")
  public HttpResponse setUserEmail(@PathParam("username") String username,
